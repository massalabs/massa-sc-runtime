use std::sync::Arc;
use wasmer::{Function, ImportObject, Instance, Module, Store, Val, imports, Cranelift, Universal, CompilerConfig};
use crate::api;
use crate::types::{Address, Response};
use anyhow::{bail, Result};
use wasmer_as::{StringPtr, Read as ASRead, Write as ASWrite, };
use crate::env::{Env, abort, get_remaining_points};
...
fn print(env: &Env, arg: i32) {
    ...
    println!("\n{}", str_ptr.read(env.wasm_env.memory.get_ref().expect("initialized memory")).unwrap());
}
...
fn call(env: &Env, address: i32, function: i32) -> i32 {
    ...
    let value = super::execution_impl::exec(env.remaining_points, None, module, fnc, &vec![]).unwrap();
...
pub fn exec(limit: u64, instance: Option<Instance>, module: &[u8], fnc: &str, params: &[i32]) -> Result<Response> {
    ...
    match instance.exports.get_function(fnc)?.call(&p) {
        Ok(value) => {
            ...
            if fnc.eq(MAIN) {
                return Ok(Response{ ret: "0".to_string(), remaining_points: 0});
            }
            let str_ptr = StringPtr::new(value.get(0).unwrap().i32().unwrap() as u32);
            let memory = instance.exports.get_memory("memory")?;
            Ok(Response { ret: str_ptr.read(memory)?, remaining_points: get_remaining_points(&instance) })
        },
