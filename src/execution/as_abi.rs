...
pub(crate) fn assembly_script_print(mut ctx: FunctionEnvMut<ASEnv>, arg: i32) -> ABIResult<()> {
    ...
    let memory = get_memory!(env);
    let message = get_string(memory, &mut ctx,  arg)?;
...
pub(crate) fn assembly_script_set_data(mut ctx: FunctionEnvMut<ASEnv>, key: i32, value: i32) -> ABIResult<()> {
    ...
    if cfg!(feature = "gas_calibration") {
        param_size_update(&env, &mut ctx, "massa.assembly_script_set_data:0", key.len(), false);
        param_size_update(&env, &mut ctx, "massa.assembly_script_set_data:1", value.len(), false);
    }
...
pub(crate) fn assembly_script_get_owned_addresses_raw(mut ctx: FunctionEnvMut<ASEnv>) -> ABIResult<i32> {
    ...
    Ok(StringPtr::alloc(&data.join(";"), env.get_wasm_env(), &mut ctx)?.offset() as i32)
}
...
pub(crate) fn assembly_script_send_message(
    ...
    let memory = get_memory!(env);
    let target_address = &get_string(memory, &mut ctx, target_address)?;
    let target_handler = &get_string(memory, &mut ctx, target_handler)?;
...
pub(crate) fn assembly_script_get_bytecode(mut ctx: FunctionEnvMut<ASEnv>) -> ABIResult<i32> {
    ...
    let env = ctx.data().clone();
    sub_remaining_gas(&env,  &mut ctx, settings::metering_get_bytecode_const())?;
...
pub(crate) fn assembly_script_get_bytecode_for(mut ctx: FunctionEnvMut<ASEnv>, address: i32) -> ABIResult<i32> {
    let env = ctx.data().clone();
    sub_remaining_gas(&env, &mut ctx, settings::metering_get_bytecode_const())?;
    let memory = get_memory!(env);
    let address = get_string(memory, &mut ctx, address)?;
...
pub(crate) fn assembly_script_local_execution(
    ...
    let env = ctx.data().clone();
    sub_remaining_gas(&env, &mut ctx, settings::metering_local_execution_const())?;
    let memory = get_memory!(env);
...
pub(crate) fn assembly_script_local_call(
    ...
    let env = ctx.data().clone();
    sub_remaining_gas(&env, &mut ctx, settings::metering_local_call_const())?;
    let memory = get_memory!(env);
...
pub fn assembly_function_exists(mut ctx: FunctionEnvMut<ASEnv>, address: i32, function: i32) -> ABIResult<i32> {
    let env = ctx.data().clone();
    sub_remaining_gas(&env, &mut ctx, settings::metering_local_call_const())?;
    let memory = get_memory!(env);
...
fn read_string_and_sub_gas(
    ...
    let env = ctx.data().clone();
    let value = StringPtr::new(offset as u32).read(memory, ctx)?;
...
fn read_buffer_and_sub_gas(
    ...
    let env = ctx.data().clone();
    let buffer = BufferPtr::new(offset as u32).read(memory, ctx)?;
