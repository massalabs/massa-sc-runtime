...
impl ContextModule {
    pub fn new(interface: &dyn Interface, gas_costs: GasCosts, binary_module: Module) -> Self {
        Self {
    ...
    pub fn create_vm_instance_and_init_env(&mut self, store: &mut Store) -> Result<Instance> {
        let (imports, fenv) = self.resolver(store);
        match Instance::new(store, &self.module, &imports) {
            Ok(i) => {
                self.init_with_instance(store, &i, &mut fenv)?;
                Ok(i)
            }
            Err(err) => {
