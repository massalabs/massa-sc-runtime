use super::abi::*;
use super::env::{get_remaining_points, set_remaining_points, ASEnv};
use crate::error::{vm_bail, VMResult};
use crate::types::Response;
use crate::{CondomLimits, GasCosts, Interface};

/// Whether the MIP-gated paginated datastore-key ABIs are exposed to guest
/// modules for a host reporting this interface version (see
/// [`crate::types::PAGINATED_DS_KEYS_EXECUTION_VERSION`]).
fn paginated_ds_keys_enabled(interface_version: u32) -> bool {
    interface_version >= crate::types::PAGINATED_DS_KEYS_EXECUTION_VERSION
}
...
impl ASContext {
    ...
    pub(crate) fn resolver(&self, store: &mut Store) -> (Imports, FunctionEnv<ASEnv>) {
        ...
        // MIP-gated ABIs: only resolve the new imports when the host reports
        // an execution component version at or past their activation, so an
        // updated node rejects the imports before activation exactly like a
        // non-updated node (otherwise the instantiation divergence forks the
        // network). An unreachable version fails closed (old behavior).
        if self
            .env
            .interface
            ...
            .is_ok_and(paginated_ds_keys_enabled)
        {
            imports.define(
                ...
                Function::new_typed_with_env(store, &fenv, assembly_script_get_keys_paginated),
            );
            imports.define(
                "massa",
                "assembly_script_get_keys_for_paginated",
                Function::new_typed_with_env(store, &fenv, assembly_script_get_keys_for_paginated),
            );
        }
...
mod tests {
    ...
    fn test_paginated_ds_keys_gate_boundaries() {
        ...
        assert!(!paginated_ds_keys_enabled(
            crate::types::PAGINATED_DS_KEYS_EXECUTION_VERSION - 1
        ));
        assert!(paginated_ds_keys_enabled(
            crate::types::PAGINATED_DS_KEYS_EXECUTION_VERSION
        ));
    ...
    /// `TestInterface` reports version 0: the new imports must not resolve
    /// (pre-activation behavior), while the deprecated ones still do.
    #[test]
    fn test_paginated_ds_keys_unresolved_before_activation() {
        ...
        let (imports, _fenv) = ctx.resolver(&mut store);

        assert!(imports.exists("massa", "assembly_script_get_keys"));
        assert!(imports.exists("massa", "assembly_script_get_keys_for"));
        assert!(!imports.exists("massa", "assembly_script_get_keys_paginated"));
        assert!(!imports.exists("massa", "assembly_script_get_keys_for_paginated"));
    }
}
