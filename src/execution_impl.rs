...
use crate::{
    env::{assembly_script_abort, get_remaining_points_for_instance, Env, ENV},
...
fn create_instance(limit: u64, module: &[u8], interface: &dyn Interface) -> Result<Instance> {
    ...
    *ENV.lock()
        .expect("Couldn't acquire mutex on SERIALIZATION_CONTEXT.") = Some(Env::new(interface));
    let resolver: ImportObject = imports! {
        "env" => {
            // Needed by wasm generated by AssemblyScript.
            "abort" =>  Function::new_native(&store, assembly_script_abort),
        },
        "massa" => {
            "assembly_script_print" => Function::new_native(&store, assembly_script_print),
            "assembly_script_call" => Function::new_native(&store, assembly_script_call_module),
            "assembly_script_get_remaining_gas" => Function::new_native(&store, assembly_script_get_remaining_gas),
            "assembly_script_create_sc" => Function::new_native(&store, assembly_script_create_sc),
            "assembly_script_set_data" => Function::new_native(&store, assembly_script_set_data),
            "assembly_script_set_data_for" => Function::new_native(&store, assembly_script_set_data_for),
            "assembly_script_get_data" => Function::new_native(&store, assembly_script_get_data),
            "assembly_script_get_data_for" => Function::new_native(&store, assembly_script_get_data_for),
            "assembly_script_has_data" => Function::new_native(&store, assembly_script_has_data),
            "assembly_script_has_data_for" => Function::new_native(&store, assembly_script_has_data_for),
            "assembly_script_get_owned_addresses" => Function::new_native(&store, assembly_script_get_owned_addresses),
            "assembly_script_get_owned_addresses_raw" => Function::new_native(&store, assembly_script_get_owned_addresses_raw),
            "assembly_script_get_call_stack" => Function::new_native(&store, assembly_script_get_call_stack),
            "assembly_script_get_call_stack_raw" => Function::new_native(&store, assembly_script_get_call_stack_raw),
            "assembly_script_generate_event" => Function::new_native(&store, assembly_script_generate_event),
            "assembly_script_transfer_coins" => Function::new_native(&store, assembly_script_transfer_coins),
            "assembly_script_transfer_coins_for" => Function::new_native(&store, assembly_script_transfer_coins_for),
            "assembly_script_get_balance" => Function::new_native(&store, assembly_script_get_balance),
            "assembly_script_get_balance_for" => Function::new_native(&store, assembly_script_get_balance_for),
            "assembly_script_hash" => Function::new_native(&store, assembly_script_hash),
            "assembly_script_signature_verify" => Function::new_native(&store, assembly_script_signature_verify),
            "assembly_script_address_from_public_key" => Function::new_native(&store, assembly_script_address_from_public_key),
            "assembly_script_unsafe_random" => Function::new_native(&store, assembly_script_unsafe_random),
            "assembly_script_get_call_coins" => Function::new_native(&store, assembly_script_get_call_coins),
            "assembly_script_get_time" => Function::new_native(&store, assembly_script_get_time),
            "assembly_script_send_message" => Function::new_native(&store, assembly_script_send_message),
            "assembly_script_get_current_period" => Function::new_native(&store, assembly_script_get_current_period),
            "assembly_script_get_current_thread" => Function::new_native(&store, assembly_script_get_current_thread),
        },
    };
