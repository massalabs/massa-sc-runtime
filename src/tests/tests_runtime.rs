...
fn test_metering_safety() {
    ...
    let runtime_module = RuntimeModule::new(bytecode, 100_000, gas_costs.clone(), Compiler::SP).unwrap();
...
fn test_run_main() {
    ...
    let runtime_module = RuntimeModule::new(module, 200_000, gas_costs.clone(), Compiler::SP).unwrap();
...
fn test_not_enough_gas_error() {
    ...
    let runtime_module = RuntimeModule::new(module, 100_000, gas_costs.clone(), Compiler::SP).unwrap();
...
fn test_wat() {
    ...
    let runtime_module = RuntimeModule::new(bytecode, gas_limit, gas_costs.clone(), Compiler::SP).unwrap();
